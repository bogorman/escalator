...
"""
  ...
  def generateAggregateEventHandler(rootTableName: String, tree: ReferenceTree): String = {
    ...
    // Group nodes by table name (keeping all FK variants)
    val nodesByTable: Map[String, List[ReferenceNode]] =
      tree.nodes.filter(!_.isWeakReference).groupBy(_.table).map { case (k, v) => k -> v.toList }

    val childEventHandlers = nodesByTable.map { case (tableName, nodes) =>
      if (nodes.size == 1) {
        // Single FK — existing behavior
        generateChildEventHandler(rootTableName, nodes.head, tree, aggregateName)
      } else {
        // Multiple FKs — generate combined handler with all branches
        generateMultiFkChildEventHandler(rootTableName, nodes, tree, aggregateName)
      }
    }.mkString("\n")

    // Generate entity ID extractor for multi-actor routing
    val entityIdExtractor = generateEntityIdExtractor(rootTableName, tree, aggregateName)
...
"""
  ...
  /**
   * Generate a combined event handler for tables with multiple FKs to the aggregate root.
   * Produces ONE apply() method with case branches for each FK side.
   */
  def generateMultiFkChildEventHandler(
    rootTableName: String,
    nodes: List[ReferenceNode],
    tree: ReferenceTree,
    aggregateName: String
  ): String = {
    val childName = namingStrategy.table(singularize(nodes.head.table))
    ...
    s"""  /**
   * Handle events for ${childName} entities (multiple FK references)
   */
...
$deleteBranches
  ...
  /**
   * Generate extractEntityIds method that returns ALL entity IDs referenced by an event.
   * For multi-FK tables (e.g. entity_members with entity_id + member_entity_id),
   * returns both IDs so events can be routed to multiple entity actors.
   */
  def generateEntityIdExtractor(rootTableName: String, tree: ReferenceTree, aggregateName: String): String = {
    val rootEntityFieldName = singularize(rootTableName.toLowerCase)

    // Root event
    val rootCase = s"""    case e: events.${aggregateName}Event => List(e.id)"""

    // Child events — group by table, emit all FK accessors
    // Some FK columns may be nullable (Option[EntityId]), so we use collect to handle both cases safely
    val nodesByTable = tree.nodes.filter(!_.isWeakReference).groupBy(_.table)
    val childCases = nodesByTable.map { case (tableName, nodes) =>
      val childName = namingStrategy.table(singularize(tableName))
      // Event trait accessor is singular lowercased with no underscores (e.g. "entitymember")
      val eventAccessor = singularize(tableName).replace("_", "").toLowerCase
      val accessors = nodes.map { node =>
        val scalaProperty = namingStrategy.column(node.foreignKeyColumn)
        s"e.$eventAccessor.$scalaProperty"
      }
      if (nodes.size > 1) {
        // Multiple FKs — use Seq with collect to handle both EntityId and Option[EntityId] safely
        val accessorList = accessors.map(a => s"        $a").mkString(",\n")
...
$accessorList
    ...
    val allCases = (List(rootCase) ++ childCases :+ "    case _ => List.empty").mkString("\n")

    s"""  /**
   * Extract ALL entity IDs from an event for multi-actor routing.
   * Events with multiple FKs to the root entity return multiple IDs.
   */
  def extractEntityIds(event: escalator.ddd.Event): List[${aggregateName}Id] = event match {
$allCases
  }"""
  }
