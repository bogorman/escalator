...
case class AttributeTypeData(attr: String, ident: String, description: String)
...
object GeneratorTemplates {
	...
	def postgresDatabaseTemplate(packageSpace: String, appName: String, tableCaseClasses: List[String]) = {
		...
		import ${packageSpace}.persistence.postgres.PostgresCustomEncoder
	...
	def modelEventTemplate(packageSpace: String, modelClass: String, table: Table) = {
		 ...
		 */
		sealed trait ${modelClass}Event extends escalator.ddd.Event with escalator.ddd.PersistentEvent {
	...
	// AttributeType generation templates
	def attributeTypesTemplate(packageName: String, className: String, attrCategory: String): String = {
		s"""package ${packageName}.models

${autoGeneratedComment}

case class ${className}(ident: String) extends BaseAttributeType {
	override def attr = "${attrCategory}"
}
"""
	}

	def attributeObjectsTemplate(packageName: String, className: String, values: List[AttributeTypeData]): String = {
		val constants = values.map { value =>
			val constantName = TextUtil.snakeToUpperCamel(value.ident.replace("-", "_"))
			s"  val ${constantName} = ${className}(\"${value.ident}\")"
		}.mkString("\n")
...
// Type-safe constants for ${className} values
...
"""
	...
	def allAttributeTypesTemplate(packageName: String, typeDefinitions: List[String]): String = {
		val allTypes = typeDefinitions.mkString("\n\n")
...
"""
	...
	def allAttributeObjectsTemplate(packageName: String, objectDefinitions: List[String]): String = {
		val allObjects = objectDefinitions.mkString("\n\n")
...
"""
	...
	def postgresCustomEncoderTemplate(packageName: String, attributeTypes: List[String]): String = {
		val encoderDecoders = attributeTypes.map { attrType =>
			val lowerName = attrType.toLowerCase.replace("type", "")
			s"""  protected lazy implicit val ${lowerName}Decoder: ctx.Decoder[${attrType}] = createAttributeTypeDecoder(${attrType}.apply)
  protected lazy implicit val ${lowerName}Encoder: ctx.Encoder[${attrType}] = createAttributeTypeEncoder[${attrType}]"""
		}.mkString("\n\n")

		s"""package ${packageName}.persistence.postgres

${autoGeneratedComment}

import ${packageName}.models._
import ${packageName}.common.persistence.postgres.PostgresCommonEncoder
