package escalator.streams.laminar.streams.operators
...
object CombineLatest {
  ...
  def apply[L, R](left$ : EventStream[L], right$ : EventStream[R]): EventStream[(L, R)] = {
    EventStream
      ...
      .foldLeft[(Option[L], Option[R])]((Option.empty[L], Option.empty[R])) { (acc: (Option[L], Option[R]), value) =>
        value match {
  ...
  def apply[L, R](left$ : Signal[L], right$ : Signal[R], startLeft: => L, startRight: => R): Signal[(L, R)] =
    EventStream
      ...
      .foldLeft((startLeft, startRight)) { (acc: (L, R), value) =>
        value match {
